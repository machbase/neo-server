...
import (
	...
	osnet "github.com/shirou/gopsutil/v4/net"
)
...
func Module(rt *goja.Runtime, module *goja.Object) {
	...
	m.Set("userInfo", func(call goja.FunctionCall) interface{} {
		options := make(map[string]interface{})
		if len(call.Arguments) > 0 && !goja.IsUndefined(call.Arguments[0]) {
			if obj := call.Argument(0).ToObject(rt); obj != nil {
				for _, key := range obj.Keys() {
					options[key] = obj.Get(key).Export()
				}
			}
		}
