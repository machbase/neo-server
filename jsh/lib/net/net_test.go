...
func TestServerCreateAndClose(t *testing.T) {
	...
	server := net.CreateServer(nil, func(obj *goja.Object, event string, args ...any) bool {
		if len(args) > 0 {
			emit(event, args[0])
		} else {
			emit(event, nil)
...
func TestClientServerConnection(t *testing.T) {
	...
	socket, err := net.Connect(nil, port, "127.0.0.1", func(obj *goja.Object, event string, args ...any) bool {
		if len(args) > 0 {
