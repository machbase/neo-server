...
func new_symDense(rt *goja.Runtime) func(c goja.ConstructorCall) *goja.Object {
...
func (d *SymDense) toValue() *goja.Object {
	obj := d.Dense.toValue()
...
func (d *SymDense) SetSym(call goja.FunctionCall) goja.Value {
...
func (d *SymDense) AddSym(call goja.FunctionCall) goja.Value {
	if len(call.Arguments) < 2 {
		return d.rt.ToValue("addSym: not enough arguments")
	}
	a, ok := call.Arguments[0].(*goja.Object).Get("$").Export().(*mat.SymDense)
	if !ok {
		return d.rt.ToValue("addSym: not a SymDense matrix")
	}
	b, ok := call.Arguments[1].(*goja.Object).Get("$").Export().(*mat.SymDense)
	if !ok {
		return d.rt.ToValue("addSym: not a SymDense matrix")
	}
	symdense := d.value.(*mat.SymDense)
...
func (d *SymDense) SubsetSym(call goja.FunctionCall) goja.Value {
	if len(call.Arguments) < 2 {
