...
func TestTagQLMajorParts(t *testing.T) {
	...
	TagQLTestCase{
		...
		run(t)
	TagQLTestCase{
		q:      "table/tag?yield=val1&yield=val2",
		...
		run(t)
	TagQLTestCase{
		q:      "table/tag?yield=" + url.QueryEscape("(val * 0.01) altVal") + "&yield=val2",
		...
		run(t)
	TagQLTestCase{
		q:      "table/tag?range=2.34s&time=last&limit=2000&yield=" + url.QueryEscape("(val + val2/2)"),
		...
		run(t)
	TagQLTestCase{
		q:      "table/tag?range=2.34s&time=now&limit=100&yield=val",
	...
	TagQLTestCase{
		...
		run(t)
	TagQLTestCase{
		q:      "table/tag?yield=" + url.QueryEscape("AVG(val1+val2)"),
...
func TestTagQLGroupBy(t *testing.T) {
	TagQLTestCase{
		...
		run(t)
	TagQLTestCase{
		q:      "table/tag?yield=" + url.QueryEscape("STDDEV(val)") + "&yield=zval&range=2.34s&time=last&group=0.5ms&limit=100",
		...
		run(t)
	TagQLTestCase{
		q:      "table/tag?yield=" + url.QueryEscape("STDDEV(val)") + "&range=2.34s&time=now&group=0.5ms&limit=100",
...
func TestTagQLErrors(t *testing.T) {
	...
	TagQLTestCase{q: "table/tag?yield=" + url.QueryEscape("val1 +* val2"), expect: "", err: "invalid token: '+*'"}.run(t)
	TagQLTestCase{q: "table/tag?yield=" + url.QueryEscape("f1(value)"), expect: "", err: "undefined function f1"}.run(t)
	TagQLTestCase{q: "table/tag?yield=value&range=1x", expect: "", err: "invalid range syntax"}.run(t)
