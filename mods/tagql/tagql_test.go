...
func TestTagQLMajorParts(t *testing.T) {
	...
	TagQLTestCase{
		...
		run(t)
	TagQLTestCase{
		q:      "table/tag?range=2.34s&time=last&limit=2000&value=" + url.QueryEscape("(val + val2/2)"),
		...
		run(t)
	TagQLTestCase{
		q:      "table/tag?range=2.34s&time=now&limit=100&value=val",
...
func TestTagQLErrors(t *testing.T) {
	...
	TagQLTestCase{q: "table/tag?value=" + url.QueryEscape("f1(value)"), expect: "", err: "undefined function f1"}.run(t)
	TagQLTestCase{q: "table/tag?value=value&range=1x", expect: "", err: "invalid range syntax"}.run(t)
