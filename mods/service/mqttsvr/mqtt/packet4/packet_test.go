...
func TestConnackConsts(t *testing.T) {
	...
	if ErrRefusedNotAuthorized != 0x05 {
		t.Errorf("Const for RefusedNotAuthorised is %d, should be %d", ErrRefusedNotAuthorized, 5)
	}
}
...
func TestConnectPacket(t *testing.T) {
	...
	packet, _, err := ReadPacket(connectPacketBytes, 1024)
	if err != nil {
...
func TestPackUnpackControlPackets(t *testing.T) {
	...
	for _, packet := range packets {
		...
		read, _, err := ReadPacket(buf, 1024)
		if err != nil {
