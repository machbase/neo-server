...
func (r *Row) Scan(cols ...any) error {
	if r.err != nil {
		return r.err
	}
	if len(cols) > len(r.values) {
		return api.ErrDatabaseScanIndex(len(cols), len(r.values))
	}
	for i := range cols {
		if err := api.Scan(r.values[i], cols[i]); err != nil {
			return err
		}
	}
	return nil
...
func (r *Rows) Scan(cols ...any) error {
	return r.sqlRows.Scan(cols...)
}
...
func (r *Rows) Columns() (api.Columns, error) {
	...
	for i, col := range cols {
		...
		ret[i].DataType = scanTypeToDataType(col.ScanType().String())
