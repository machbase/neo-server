...
func ValidateScriptStructure(script *TQLScript) error {
	...
	if err := validateStatementPlacement(head, StageSource); err != nil {
		return err
	}
	if err := validateStatementPlacement(tail, StageSink); err != nil {
		return err
	}

	for _, stmt := range codes[1 : len(codes)-1] {
		if err := validateStatementPlacement(stmt, StageMap); err != nil {
			return err
		}
	}
...
func validateStatementPlacement(stmt *Statement, role StageRole) error {
	if stmt == nil || isApplicableForRole(stmt.Kind, role) {
		return nil
	}
	return newScriptError(
		invalidPlacementKind(role),
		stmt,
		fmt.Sprintf("%q is not applicable for %s (allowed: %s)", stmt.Name, stageRoleLabel(role), statementKindAllowedRoles(stmt.Kind)),
		nil,
	)
}

func invalidPlacementKind(role StageRole) string {
	switch role {
	case StageSource:
...
func isApplicableForRole(kind StatementKind, role StageRole) bool {
	switch role {
	case StageSource:
		return isApplicableForSource(kind)
	case StageMap:
		return isApplicableForMap(kind)
	case StageSink:
		return isApplicableForSink(kind)
	default:
		return false
	}
}

func stageRoleLabel(role StageRole) string {
	switch role {
	case StageSource:
		return "SRC"
	case StageMap:
...
func statementKindAllowedRoles(kind StatementKind) string {
	roles := make([]string, 0, 3)
	if isApplicableForSource(kind) {
		roles = append(roles, stageRoleLabel(StageSource))
	}
	if isApplicableForMap(kind) {
		roles = append(roles, stageRoleLabel(StageMap))
	}
	if isApplicableForSink(kind) {
		roles = append(roles, stageRoleLabel(StageSink))
	}
	if len(roles) == 0 {
		return "none"
	}
