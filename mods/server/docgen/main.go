...
func main() {
	...
	markdown, err := renderMarkdown(registrations, moduleRoot)
	if err != nil {
		panic(err)
	}
...
func renderMarkdown(registrations []registeredMethod, moduleRoot string) ([]byte, error) {
	buffer := &bytes.Buffer{}
	importedPackages := map[string]*packageInfo{}
	...
	for groupIndex, group := range groupRegistrations(registrations) {
		...
		for _, registration := range group.Methods {
			renderMethod(buffer, registration, moduleRoot, importedPackages)
		}
	}
...
func renderMethod(buffer *bytes.Buffer, registration registeredMethod, moduleRoot string, importedPackages map[string]*packageInfo) {
	...
	if len(params) == 0 {
		...
		for index, param := range params {
			...
			entries := paramSchemaEntries(name, param, registration.Package, moduleRoot, importedPackages)
	...
	buffer.WriteString("\n*Return*\n\n")
	renderReturnLines(buffer, returns, registration.Function.ReturnDoc, registration.Function.ReturnFieldDoc, registration.Package, moduleRoot, importedPackages)

	request, response := sampleMessages(registration.Name, params, returns, registration.Package, moduleRoot, importedPackages)
	buffer.WriteByte('\n')
...
func paramSchemaEntries(paramName string, param fieldInfo, pkg *packageInfo, moduleRoot string, importedPackages map[string]*packageInfo) []schemaEntry {
	if pkg == nil {
		return nil
	}
	entries := []schemaEntry{}
	collectSchemaEntries(&entries, pkg, param.Expr, paramName, map[string]bool{}, moduleRoot, importedPackages)
	return entries
}

func collectSchemaEntries(entries *[]schemaEntry, pkg *packageInfo, expr ast.Expr, prefix string, visited map[string]bool, moduleRoot string, importedPackages map[string]*packageInfo) {
	structName, structInfo, ok := resolveStructType(pkg, expr, moduleRoot, importedPackages)
	...
	for _, field := range structInfo.Fields {
		...
		if isStructLikeField(pkg, field.Expr, moduleRoot, importedPackages) {
			fieldType = "object"
		}
		*entries = append(*entries, schemaEntry{
		...
		collectSchemaEntries(entries, pkg, field.Expr, path, visited, moduleRoot, importedPackages)
	}
	delete(visited, structName)
}

func isStructLikeField(pkg *packageInfo, expr ast.Expr, moduleRoot string, importedPackages map[string]*packageInfo) bool {
	_, _, ok := resolveStructType(pkg, expr, moduleRoot, importedPackages)
	return ok
}

func resolveStructType(pkg *packageInfo, expr ast.Expr, moduleRoot string, importedPackages map[string]*packageInfo) (string, *structInfo, bool) {
	if pkg == nil {
		return "", nil, false
	}
	switch typed := expr.(type) {
	case *ast.StarExpr:
		return resolveStructType(pkg, typed.X, moduleRoot, importedPackages)
	case *ast.Ident:
	...
	case *ast.SelectorExpr:
		...
		importPath, ok := pkg.Imports[owner.Name]
		if !ok {
			return "", nil, false
		}
		...
		importedPkg, err := loadImportedPackage(importPath, importedPackages, moduleRoot)
		if err != nil {
			return "", nil, false
		}
		st, ok := importedPkg.Structs[typed.Sel.Name]
		if !ok {
			return "", nil, false
		}
		return importedPkg.Name + "." + typed.Sel.Name, st, true
	default:
		return "", nil, false
	}
}
...
func renderReturnLines(buffer *bytes.Buffer, returns []fieldInfo, docs map[string]string, fieldDocs map[string][]docEntry, pkg *packageInfo, moduleRoot string, importedPackages map[string]*packageInfo) {
	...
	for idx, line := range lines {
		...
		docEntries := fieldDocs[key]
		if returns[idx].Name != "" {
			if namedEntries := fieldDocs[returns[idx].Name]; len(namedEntries) > 0 {
				docEntries = namedEntries
			}
		}

		autoEntries := returnSchemaEntries(returns[idx], pkg, moduleRoot, importedPackages)
		docByName := map[string]string{}
		for _, entry := range docEntries {
			docByName[entry.Name] = entry.Desc
		}
		renderedByDoc := map[string]bool{}
		...
		for _, entry := range autoEntries {
			...
			if desc, ok := docByName[entry.Path]; ok && strings.TrimSpace(desc) != "" {
				fmt.Fprintf(buffer, "  - `%s` *%s* - %s\n", entry.Path, typ, desc)
				renderedByDoc[entry.Path] = true
				continue
			}
			fmt.Fprintf(buffer, "  - `%s` *%s*\n", entry.Path, typ)
		}
...
func returnSchemaEntries(ret fieldInfo, pkg *packageInfo, moduleRoot string, importedPackages map[string]*packageInfo) []schemaEntry {
	if pkg == nil {
		return nil
	}
	entries := []schemaEntry{}
	collectReturnSchemaEntries(&entries, pkg, ret.Expr, "", map[string]bool{}, moduleRoot, importedPackages)
	return entries
}

func collectReturnSchemaEntries(entries *[]schemaEntry, pkg *packageInfo, expr ast.Expr, prefix string, visited map[string]bool, moduleRoot string, importedPackages map[string]*packageInfo) {
	switch typed := expr.(type) {
	case *ast.ArrayType:
		nextPrefix := "[]"
		if prefix != "" {
		...
		collectReturnSchemaEntries(entries, pkg, typed.Elt, nextPrefix, visited, moduleRoot, importedPackages)
		return
	}
	...
	for _, field := range structInfo.Fields {
		...
		*entries = append(*entries, schemaEntry{
		...
		collectReturnSchemaEntries(entries, pkg, field.Expr, path, visited, moduleRoot, importedPackages)
	}
	delete(visited, structName)
}
...
func sampleMessages(method string, params []fieldInfo, returns []fieldInfo, pkg *packageInfo, moduleRoot string, importedPackages map[string]*packageInfo) (rpcRequestEnvelope, rpcResponseEnvelope) {
	sampleParams := make([]any, 0, len(params))
	for _, param := range params {
		sampleParams = append(sampleParams, sampleParamValue(param.Expr, param.Type, pkg, map[string]bool{}, moduleRoot, importedPackages))
	}
...
func sampleParamValue(expr ast.Expr, fallback string, pkg *packageInfo, visited map[string]bool, moduleRoot string, importedPackages map[string]*packageInfo) any {
	if pkg != nil {
		if structName, st, ok := resolveStructType(pkg, expr, moduleRoot, importedPackages); ok {
			if visited[structName] {
			...
			for _, field := range st.Fields {
				obj[field.Name] = sampleParamValue(field.Expr, field.Type, pkg, visited, moduleRoot, importedPackages)
			}
			delete(visited, structName)
			return obj
		}
	...
	switch typed := expr.(type) {
	case *ast.StarExpr:
		return sampleParamValue(typed.X, renderExpr(typed.X), pkg, visited, moduleRoot, importedPackages)
	case *ast.ArrayType:
		return []any{}
	case *ast.MapType:
		return map[string]any{}
