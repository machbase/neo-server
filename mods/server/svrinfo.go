...
type SessionWatcher interface {
	ListWatcher(cb func(*mach.ConnWatcher) bool)
}
...
func (s *svr) ServerSessions(reqStatz, reqSessions bool) (statz *machrpc.Statz, sessions []*machrpc.Session, err error) {
	...
	if reqSessions {
		...
		if sw, ok := s.db.(SessionWatcher); ok {
			sw.ListWatcher(func(w *mach.ConnWatcher) bool {
				lsql, ltime := w.LatestSql()
				sessions = append(sessions, &machrpc.Session{
					Key:           w.Key,
					CreTime:       w.Created.UnixNano(),
					LatestSql:     lsql,
					LatestSqlTime: ltime.UnixNano(),
				})
				return true
			})
		}
