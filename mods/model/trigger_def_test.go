...
type fakeScheduleStore struct {
	...
	createTables              int
	dropTables                int
	missingTimerUserName      bool
	missingSubscriberUserName bool
}
...
func (c *fakeScheduleConn) ExecContext(_ context.Context, query string, args []driver.NamedValue) (driver.Result, error) {
	...
	case strings.HasPrefix(q, "DROP TABLE _NEO_TIMER_DEF"):
		c.store.mu.Lock()
		c.store.dropTables++
		c.store.missingTimerUserName = false
	...
	case strings.HasPrefix(q, "DROP TABLE _NEO_SUBSCRIBER_DEF"):
		c.store.mu.Lock()
		c.store.dropTables++
		c.store.missingSubscriberUserName = false
	...
	case strings.HasPrefix(q, "INSERT INTO _NEO_TIMER_DEF"):
		...
		c.store.nextTimerID++
		c.store.timers[id] = fakeTimerRow{id: id, userName: stringValue(args[0]), name: stringValue(args[1]), execUser: stringValue(args[2]), autoStart: intValue(args[3]), disabled: intValue(args[4]), lastError: stringValue(args[5]), lastErrorAt: timeValue(args[6]), task: stringValue(args[7]), schedule: stringValue(args[8]), attributes: stringValue(args[9])}
	...
	case strings.HasPrefix(q, "INSERT INTO _NEO_SUBSCRIBER_DEF"):
		...
		c.store.nextSubID++
		c.store.subs[id] = fakeSubscriberRow{id: id, userName: stringValue(args[0]), name: stringValue(args[1]), execUser: stringValue(args[2]), autoStart: intValue(args[3]), disabled: intValue(args[4]), lastError: stringValue(args[5]), lastErrorAt: timeValue(args[6]), task: stringValue(args[7]), bridge: stringValue(args[8]), topic: stringValue(args[9]), qos: intValue(args[10]), queueName: stringValue(args[11]), streamName: stringValue(args[12]), attributes: stringValue(args[13])}
		return fakeScheduleResult{lastInsertID: id, rowsAffected: 1}, nil
	case strings.HasPrefix(q, "UPDATE _NEO_TIMER_DEF SET LAST_ERROR"):
		...
		row.lastError, row.lastErrorAt = stringValue(args[0]), timeValue(args[1])
	...
	case strings.HasPrefix(q, "UPDATE _NEO_TIMER_DEF"):
		...
		id := intValue(args[9])
		row, ok := c.store.timers[id]
		if !ok || row.userName != stringValue(args[10]) {
			return fakeScheduleResult{}, nil
		}
		row.name, row.execUser, row.autoStart, row.disabled, row.lastError, row.lastErrorAt, row.task, row.schedule, row.attributes = stringValue(args[0]), stringValue(args[1]), intValue(args[2]), intValue(args[3]), stringValue(args[4]), timeValue(args[5]), stringValue(args[6]), stringValue(args[7]), stringValue(args[8])
		c.store.timers[id] = row
		return fakeScheduleResult{rowsAffected: 1}, nil
	case strings.HasPrefix(q, "UPDATE _NEO_SUBSCRIBER_DEF"):
		...
		id := intValue(args[13])
		row, ok := c.store.subs[id]
		if !ok || row.userName != stringValue(args[14]) {
			return fakeScheduleResult{}, nil
		}
		row.name, row.execUser, row.autoStart, row.disabled, row.lastError, row.lastErrorAt, row.task, row.bridge, row.topic, row.qos, row.queueName, row.streamName, row.attributes = stringValue(args[0]), stringValue(args[1]), intValue(args[2]), intValue(args[3]), stringValue(args[4]), timeValue(args[5]), stringValue(args[6]), stringValue(args[7]), stringValue(args[8]), intValue(args[9]), stringValue(args[10]), stringValue(args[11]), stringValue(args[12])
		c.store.subs[id] = row
		return fakeScheduleResult{rowsAffected: 1}, nil
	case strings.HasPrefix(q, "DELETE FROM _NEO_TIMER_DEF"):
		...
		if strings.Contains(q, "WHERE ID") {
			...
			deleted = ok && (len(args) == 1 || row.userName == stringValue(args[1]))
			if deleted {
...
func (c *fakeScheduleConn) QueryContext(_ context.Context, query string, args []driver.NamedValue) (driver.Rows, error) {
	...
	case strings.Contains(q, "FROM _NEO_TIMER_DEF WHERE ID = ? AND USER_NAME = ?"):
		row, ok := c.store.timers[intValue(args[0])]
		if !ok || row.userName != stringValue(args[1]) {
...
func timerRows(row fakeTimerRow) driver.Rows {
	return &fakeScheduleRows{columns: timerColumns, values: [][]driver.Value{{row.id, row.userName, row.name, row.execUser, row.autoStart, row.disabled, row.lastError, row.lastErrorAt, row.task, row.schedule, row.attributes}}}
}
...
func subscriberRows(row fakeSubscriberRow) driver.Rows {
	return &fakeScheduleRows{columns: subscriberColumns, values: [][]driver.Value{{row.id, row.userName, row.name, row.execUser, row.autoStart, row.disabled, row.lastError, row.lastErrorAt, row.task, row.bridge, row.topic, row.qos, row.queueName, row.streamName, row.attributes}}}
}
...
func timeValue(v driver.NamedValue) time.Time {
...
func TestScheduleTablePersistence(t *testing.T) {
	...
	require.True(t, loadedTimer.Disabled)
	require.Equal(t, "timer failed", loadedTimer.LastError)
	...
	require.True(t, loadedSub.Disabled)
	require.Equal(t, "subscriber failed", loadedSub.LastError)
...
func TestScheduleTableResetForMissingUserName(t *testing.T) {
	provider, store, cleanup := newFakeScheduleProvider(t)
	defer cleanup()
	store.missingTimerUserName = true
	store.missingSubscriberUserName = true
	ctx := context.Background()
	require.NoError(t, provider.SaveTimer(ctx, &TimerDefinition{UserName: "sys", Name: "timer", ExecUser: "sys", Task: "task", Schedule: "@every 1m"}))
	require.NoError(t, provider.SaveSubscriber(ctx, &SubscriberDefinition{UserName: "sys", Name: "subscriber", ExecUser: "sys", Task: "task", Bridge: "mqtt", Topic: "topic"}))
	require.Equal(t, 2, store.dropTables)
}
...
func TestScheduleNameUniquenessAndNotFound(t *testing.T) {
	...
	defer cleanup()
	ctx := context.Background()
	def := &TimerDefinition{UserName: "sys", Name: "same", ExecUser: "sys", Task: "a", Schedule: "@every 1m"}
	require.NoError(t, provider.SaveTimer(ctx, def))
	sub := &SubscriberDefinition{UserName: "sys", Name: "same", ExecUser: "sys", Task: "a", Bridge: "b", Topic: "t"}
