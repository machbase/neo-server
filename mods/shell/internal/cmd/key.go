...
func pcKey() action.PrefixCompleterInterface {
	return action.PcItem("key")
}

func doKey(ctx *action.ActionContext) {
	cmd := &KeyCmd{}
	parser, err := action.Kong(cmd, func() error { ctx.Println(helpKey); cmd.Help = true; return nil })
	if err != nil {
...
func doKeyList(ctx *action.ActionContext) {
	mgmtCli, err := ctx.Actor.ManagementClient()
	if err != nil {
		ctx.Println("ERR", err.Error())
		return
	}
	rsp, err := mgmtCli.ListKey(ctx.Ctx, &mgmt.ListKeyRequest{})
	if err != nil {
		ctx.Println("ERR", err.Error())
		return
	}
...
func doKeyDel(ctx *action.ActionContext, id string) {
	mgmtCli, err := ctx.Actor.ManagementClient()
	if err != nil {
		ctx.Println("ERR", err.Error())
		return
	}
	rsp, err := mgmtCli.DelKey(ctx.Ctx, &mgmt.DelKeyRequest{
		Id: id,
	})
	if err != nil {
		ctx.Println("ERR", err.Error())
...
func doKeyGen(ctx *action.ActionContext, name string, output string) {
	...
	rsp, err := mgmtCli.GenKey(ctx.Ctx, &mgmt.GenKeyRequest{
		Id:        name,
		Type:      "ec",
...
func doServerCert(ctx *action.ActionContext, output string) {
	mgmtCli, err := ctx.Actor.ManagementClient()
	if err != nil {
		ctx.Println("ERR", err.Error())
		return
	}
	rsp, err := mgmtCli.ServerKey(ctx.Ctx, &mgmt.ServerKeyRequest{})
	if err != nil {
		ctx.Println("ERR", err.Error())
		return
	}
