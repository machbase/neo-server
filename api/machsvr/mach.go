...
func Finalize() {
	...
	if singletonEnv.handle != nil {
		mach.EngFinalize(singletonEnv.handle)
	}
}
...
func DestroyDatabase() error {
	...
	return mach.EngDestroyDatabase(singletonEnv.handle)
}
...
func CreateDatabase() error {
	...
	return mach.EngCreateDatabase(singletonEnv.handle)
}
...
func ExistsDatabase() bool {
	...
	return mach.EngExistsDatabase(singletonEnv.handle)
}

func RestoreDatabase(path string) error {
	return mach.EngRestoreDatabase(singletonEnv.handle, path)
}
...
func StatzSnapshot() *Statz {
	ret := statz
	if singletonEnv.handle != nil {
		ret.RawConns = int32(mach.EngConnectionCount(singletonEnv.handle))
	}
	return &ret
}
