...
func (row *Row) Scan(cols ...any) error {
	...
	for i := range cols {
		...
		var isNull bool
		switch src := row.values[i].(type) {
		case *int16:
			row.err = types.ScanInt16(*src, cols[i])
		case *int32:
			row.err = types.ScanInt32(*src, cols[i])
		case *int64:
		...
		case *float64:
			row.err = types.ScanFloat64(*src, cols[i])
		case *net.IP:
			row.err = types.ScanIP(*src, cols[i])
		case *string:
			row.err = types.ScanString(*src, cols[i])
		case []byte:
			row.err = types.ScanBytes(src, cols[i])
		default:
			if src == nil {
				cols[i] = nil
			} else {
				return types.ErrDatabaseScanType(fmt.Sprintf("column[%d]", i), src)
			}
		}
		if row.err != nil {
			return row.err
...
func (rows *Rows) Fetch() ([]any, bool, error) {
	...
	for i := range values {
		values[i], err = rows.columns[i].DataType.MakeBuffer()
		if err != nil {
			return nil, next, types.ErrDatabaseUnsupportedTypeName("Fetch", string(rows.columns[i].DataType))
		}
	}
	for i := range values {
...
func readColumnData(stmt unsafe.Pointer, rawType int, idx int, dst any) error {
	...
	case ColumnRawTypeFloat32:
		...
		if nonNull {
			return types.ScanFloat32(v, dst)
		}
	case ColumnRawTypeFloat64:
		...
		if nonNull {
			return types.ScanFloat64(v, dst)
		}
