...
#define MACH_ENGINE_NULLABLE_UNKNOWN  2

#define MACH_ENGINE_DECIMAL_SIZE 32
typedef struct MachEngineDecimal
{
    unsigned char mOpaque[MACH_ENGINE_DECIMAL_SIZE];
} MachEngineDecimal;
...
typedef struct MachEngineColumnInfoV2
{
    char    mColumnName[MACH_ENGINE_COLUMN_NAME_MAX_LENGTH+1];
    int     mColumnType;
    int     mPrecision;
    int     mScale;
    int     mNullable;
    int     mPrimary;
    int     mColumnLength;
} MachEngineColumnInfoV2;

typedef struct MachEngineParamInfo
{
    char    mParamName[MACH_ENGINE_COLUMN_NAME_MAX_LENGTH+1];
    int     mIsNamed;
    int     mType;
    int     mPrecision;
    int     mScale;
...
/* DECIMAL bind는 mVar에 canonical decimal 문자열을 지정한다. */
typedef union MachEngineBindParamData
{
    short                           mShort;
    unsigned short                  mUShort;
    int                             mInteger;
...
int MachConnectEx(void*  aEnvHandle,
                  char*  aUserName,
                  char*  aPassword,
                  char*  aDatabase,
                  void** aConHandle);
...
int MachConnectNoAuthEx(void*  aEnvHandle,
                        char*  aUserName,
                        char*  aDatabase,
                        void** aConHandle);

int MachSetCurrentDatabase(void* aConHandle, char* aDatabase);
int MachGetCurrentDatabase(void* aConHandle, char* aBuffer, int aBufferSize);
 ...
 *
 * Statement type values are stable protocol identifiers and are never reused.
 * DDL: 1-29
 * BACKUP/RESTORE/MOUNT: 100-108
 * DCL/ALTER SYSTEM/TRANSACTION CONTROL: 256-281, 489
 * SELECT: 512
 * INSERT: 513
 * DELETE: 514-518
 * INSERT_SELECT: 519
...
int MachColumnInfoV2(void* aMachStmt, int aColumnIndex, MachEngineColumnInfoV2* aColumnInfo);
...
int MachDecimalFromUInt64(void* aMachStmt, MachEngineDecimal* aDecimal, unsigned long long aValue);
int MachDecimalFromDouble(void* aMachStmt, MachEngineDecimal* aDecimal, double aValue);
int MachDecimalSetNull(void* aMachStmt, MachEngineDecimal* aDecimal);
...
int MachBindParam(void* aMachStmt, int aParamNo, MachEngineBindParam* aParam);
int MachBindParamByName(void* aMachStmt,
                        char* aParamName,
                        int aParamNameLength,
                        MachEngineBindParam* aParam);
int MachParamCount(void* aMachStmt, int* aParamCount);
int MachParamInfo(void* aMachStmt, int aParamNo, MachEngineParamInfo* aParamInfo);

int MachGetGeneratedRowID(void* aMachStmt,
                          unsigned long long* aRowID,
                          int* aHasValue);
